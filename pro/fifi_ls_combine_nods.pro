...
; 
;-
function fifi_ls_combine_nods, fileinfo, verbose, $
...
if n_elements(write_same_position_to_one) eq 0 then write_same_position_to_one = 1
...
while (i lt n_elements(filenames)) do begin
   ...
   if (nodstyle eq 'asymmetric') then begin
      print, 'running comb nods asym on files=',filenames
...
while ii lt n_uniq_rows do begin 
   ...
   while kk lt n_elements(datalist) do begin ; kk is looping over all file extensions
      ; recall datalist has:
      ; filename, channel, dlam, dbet, indpos, nod_beam, data, sigma,$
      ; primehead, exthdr
   ...
   if (combined gt 1) then begin
      ...
      ;    outfileroot = outfilestr[0]
     ; 
     ;     outfile = str_replace(outfileroot,'.chopsub.fits', '.nodcomb.fits')
      ;    outfile = str_replace(outfile, 'A_', '')
      ;    outfile = str_replace(outfile, 'B_', '')
