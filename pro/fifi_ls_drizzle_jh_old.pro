...
function fifi_ls_drizzle_jh, infile, new_n_pix_x = new_n_pix_x, $
  ...
  if (channel eq 'RED') then begin
     deltax = 12L
     deltay = 12L
     if ~keyword_set(delta_x_out) then delta_x_out = 12
     if ~keyword_set(delta_y_out) then delta_y_out = 12
  endif else begin
     deltax = 6L
     deltay = 6L
     if ~keyword_set(delta_x_out) then delta_x_out = 6
     if ~keyword_set(delta_y_out) then delta_y_out = 6
  endelse
  ...
  if dlam_map gt 0 then begin
     x_shift = -dlam_map / (delta_x_out)
     print, 'x_shift=', x_shift
  endif
  
  if dbet_map gt 0 then begin
     y_shift = dbet_map / (delta_y_out)
  ...
  while (i lt 5) do begin
     ...
     while (j lt 5) do begin
        ...
        ;if (j eq 1) and (i eq 1) then begin
         ;  lp = rlp[2,2,*]
         ;  fp = rfp[2,2,*]
        ;endif else begin
           lp = rlp[i,j,*] 
  ...
  lambdas = reform(hypercube[*,*,0,*], 400*n_ext)
  fluxes = reform(hypercube[*,*,1,*], 400*n_ext)
  xes = reform(rxs, 400*n_ext)
  yes = reform(rys, 400*n_ext)
 ...
 ; x_grid = reverse(x_grid) + delta_x_out / 2.
  ...
  bfg = dblarr(400 * n_ext)

  ; do the drizzle
  for spax=0, (400 * n_ext)-1 do begin
     flux = fluxes[spax]
     ...
     if count ne 0 then begin
        if xcenter[0] lt 1 then begin
           ...
           xstart = xcenter[0] - 1
        endelse
        if xcenter[-1] + 1 ge n_elements(x_grid)-1 then begin
           xend = xcenter[-1]
        endif else begin
           xend = xcenter[-1] + 1
        endelse  
     ...
     if ycount ne 0 then begin
        if ycenter[0] lt 1 then begin
           ...
           ystart = ycenter[0]-1
        endelse
        if ycenter[-1] + 1 ge n_elements(y_grid)-1 then begin
           yend = ycenter[-1]
        endif else begin
           yend = ycenter[-1] + 1
        endelse  
     ...
     if debug then begin
        ...
        print, 'x0, x1, lhs_x, rhs_x=', x0, x1, lhs_x, rhs_x
        ...
        for j = ystart, yend do begin
           ...
           for k=lambdastart, lambdaend do begin
              ...
              if volume gt 0 and finite(flux) and (flux gt 0) then begin
                 big_flux_grid[i, j, k] = big_flux_grid[i, j, k] + $
                                          (flux * volume / orig_volume)
                          if big_flux_grid[i,j,k] gt 10 then begin
                          ...
                          ;bfg[spax] = bfg[spax] + (flux * volume / orig_volume)
  ...
  print,'dlam_map=',dlam_map
  print,'dbet_map=',dbet_map
  print,'x_shift=',x_shift
  print,'y_shift=',y_shift
;
 ; x_shift = - dlam_map
 ; y_shift = dbet_map
  ;for hh=0, n_elements(big_flux_grid[*, 0, 0]) -1 do begin
  ;   for kk=0, n_elements(big_flux_grid[0, *, 0]) -1 do begin
        
  for ll=0, n_elements(big_flux_grid[0, 0, *])-1 do begin
   if (x_shift ne 0) then begin
    big_flux_grid[*, *, ll] = shift(big_flux_grid[*, *, ll], x_shift)
   endif
  if (y_shift ne 0) then begin
     big_flux_grid[*, *, ll] = shift(big_flux_grid[*, *, ll], 0,  y_shift)
  endif
  endfor
 ; endfor
  ...
  for j=new_n_pix_y-1,0,-1 do begin
        for i=0, new_n_pix_x-1 do  begin
           plot,lambda_grid, big_flux_grid[i, j, *],psym=2;, yrange=[0,10]
        endfor
  endfor
  ...
  for i=0, n_elements(x_grid) - 1 do begin
     ...
     if (i eq 0) and (j eq 0) then begin
         plot, [x_grid[i]], [y_grid[j]], psym=6, linestyle=3, color=100, xrange=[-50, 50], yrange=[-50, 50]
  ...
  mwrfits, big_flux_grid, outfilename
                                
  of2 = infile+'.predriz_flux.fits'
  if file_test(of2) gt 0 then file_delete, of2
  fxwrite, of2, primehead, /noupdate
  mwrfits, reform(fluxes, 5, 5, 16, n_ext), of2
